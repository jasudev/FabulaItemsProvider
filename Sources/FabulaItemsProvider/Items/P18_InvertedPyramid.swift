...
public struct P18_InvertedPyramid: View {
        ...
        GeometryReader { proxy in
            ZStack {
                ...
                HStack(alignment: .bottom, spacing: hspacing) {
                    InvPyramidGraphView(percentage: $percentage1,
                    ...
                    InvPyramidGraphView(percentage: $percentage2,
                    ...
                    InvPyramidGraphView(percentage: $percentage3,
...
fileprivate
struct InvPyramidGraphView: View {
...
fileprivate
struct PyramidTopShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        let startPoint = CGPoint(x: 0, y: 0)
        let centerPoint = CGPoint(x: rect.width * 0.33, y: rect.width * 0.2)
...
fileprivate
struct InvPyramidView: View {
    var color = Color.red
    var body: some View {
        ZStack{
            InvPyramid(color: color)
            
            GeometryReader { proxy in
                ZStack {
                    InvPyramid(color: Color.black)
                        .scaleEffect(CGSize(width: -0.7, height: 1))
                        .opacity(0.7)
                        .blur(radius: 1)
                        .mask(
                            LinearGradient(gradient: Gradient(colors: [Color.black.opacity(0.3), Color.clear]), startPoint: .bottom, endPoint: .top)
                        )
                        .rotation3DEffect(Angle(degrees: 110), axis: (x: 0, y: 0, z: 1), anchor: .bottom)
                }
            }
        }
    }
...
struct P18_InvertedPyramid_Previews: PreviewProvider {
